...
**재배포 절차:** `./scripts/deploy.sh "커밋 메시지"` — ⚠️ 아래를 한 줄 셸 체인으로 직접 실행하지 말 것(중간 실패 시 dist가 소스 레포 루트에 커밋되는 사고가 실제로 발생). 스크립트는 임시 디렉터리에서만 작업한다.
```bash
...
- 사이트 라이브, **통합 진단(/check)**, **지역 디렉터리(/regions, 시·도 16페이지)**, 계산기 **6종**(근로장려금·자녀장려금·출산육아·청년월세·실업급여·기준중위소득, 계산 검증 완료), 정보 글 **25개**.
...
## 4. 전략 방향 (2026-07-27 결정)

**"제도 하나당 계산기 하나"로는 공식 사이트를 못 이긴다.** 홈택스 모의계산은 로그인 없이 근로·자녀장려금을 계산해 주고, 복지로는 복지사업 모의계산을 제공한다. 그래서 경쟁 지점을 **공식이 구조적으로 못 하는 것**으로 옮겼다.

- ✅ **통합 진단 `/check`** — 한 번 입력 → 근로·자녀장려금, 기초생활 4대 급여, 주거급여, 청년월세, 부모급여·아동수당, 지역 사업을 한 화면에서 판정 + **중복 수급 가능 여부**까지 안내. 기관이 쪼개져 있어 공식 사이트는 이걸 못 한다. 홈 히어로·헤더 첫 메뉴가 여기로 간다.
- 산정 로직·상수는 **`src/lib/benefits.js` 단일 소스**. 개별 계산기(근로·자녀·중위소득)도 이 파일을 import 한다. **제도값을 고칠 때는 이 파일만 고치고, 관련 글의 수치와 어긋나지 않는지 확인할 것.**
- 정책 테마의 구조적 약점(YMYL 상위노출 불리 · 5·9월 시즌 쏠림 · 매년 갱신 부채 · 낮은 CPC)은 남아 있다. 완화책으로 **급여·세금 계산기(실수령액·퇴직금·연말정산·4대보험) 확장**이 후보로 논의됨 — 검색량이 크고 연중 고르며 민간 사이트가 상위를 먹는 게 증명된 영역. 아직 착수 안 함.
- ⏸️ **gov24 분야별 페이지는 보류** — 복지로 요약 복제라 위 약점을 하나도 해결하지 못한다. 수집한 10,981건(`src/data/benefits-gov24.json`)은 통합 진단의 재료로 쓰는 편이 낫다.

## 5. 성장 작업 (다음 계정/AI가 진행할 실제 과제)
